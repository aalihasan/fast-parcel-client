...
// Remove: import { useLoaderData } from "react-router";
...
const SendParcel = () => {
  ...
  // Replace useLoaderData with useState + useEffect to fetch JSON from public folder
  ...
  useEffect(() => {
    fetch('/serviceCenter.json')
      ...
      .catch(err => console.error('Error loading serviceCenter.json:', err));
  }, []);

  // Now use serviceCenters safely
        ...
        axiosSecure
          ...
          .then(res => {
            ...
            if (res.data.insertedId) {
              ...
              // TODO: Add redirect logic here if needed
            }
          })
          .catch(err => {
            Swal.fire({
              icon: 'error',
              title: 'Error',
              text: 'Failed to submit parcel. Please try again.',
            });
            console.error(err);
          });
      }
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        {/* Submit Button */}
        <div className="text-center">
          <button className="btn btn-primary ">Send Parcel</button>
        </div>
      </form>
    </div>
  );
